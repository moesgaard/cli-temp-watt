...
RESULTS=$( sensors )
echo -e "Watt usage:"
BOB=$( echo  -e  "$RESULTS" | grep -E "(SVI2_P_Core|SVI2_P_SoC|W)" )
ZOBE=0
for ill in $BOB
do
...
ZOB=0
BOB=$(  echo  -e  "$RESULTS" | grep "T" | grep -v "PPT" | grep -v "VTT" | grep -v "EC_"   )
echo -e "\nCPU:"
for ill in $BOB
do
	ZOB=$( echo $ill | awk '{print $2}' | grep -Eo '[0-9]*\.[0-9]*' | cut -d. -f1 | xargs )
	VOV=""
	BOBBY=$( echo $ill | awk '{print $1}'  | sed  "s/_/ /g"  )
	for ins in $( seq 0 $ZOB )
		do
		VOV+=$BOF
	done
...
echo -e  "\nGPU:"
BOB=$(  echo  -e  "$RESULTS" | grep -E  "(junction|edge|mem)")
for ill in $BOB
do
...
echo -e  "\nTemperature:"
BOB=$(  echo  -e  "$RESULTS" | grep -E  "sensor")
for ill in $BOB
do
	ZOB=$( echo $ill | awk '{print $2}' | grep -E '[0-9]*\.[0-9]*' | cut -d. -f1  )
	VOV=""
